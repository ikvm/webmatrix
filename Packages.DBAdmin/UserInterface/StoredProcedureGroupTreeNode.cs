namespace Microsoft.Matrix.Packages.DBAdmin.UserInterface
{
    ...
    internal class StoredProcedureGroupTreeNode : DataTreeNode
    {
        public StoredProcedureGroupTreeNode(ProjectItem item) : base(item, 9, 10)
        {
        ...
        internal void AddItem()
        {
            ...
            if (item != null)
            {
                ...
                {
                    base.Nodes.Add(new StoredProcedureTreeNode(item));
                }
                IDocumentManager service = (IDocumentManager) ((IServiceProvider) base.ProjectItem.Project).GetService(typeof(IDocumentManager));
                if (service != null)
                {
                    service.OpenDocument((DocumentProjectItem) item, false, DocumentViewType.Default);
                }
                if (!base.IsExpanded)
                {
                    base.Expand();
