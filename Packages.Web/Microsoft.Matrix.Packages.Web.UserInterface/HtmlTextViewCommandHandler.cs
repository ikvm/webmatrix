namespace Microsoft.Matrix.Packages.Web.UserInterface
{
    ...
    public class HtmlTextViewCommandHandler : ICommandHandler, IDisposable
    {
        ...
        private void BlockUncomment(TextBufferSpan span)
        {
            using (TextBufferSpan span2 = this.ShrinkToText(span))
            {
                ...
                try
                {
                    if (((span2.Start.ColumnIndex < (span2.Start.Line.Length - 3)) && (span2.Start.Line.Substring(span2.Start.ColumnIndex, 4) == "<!--")) && ((span2.End.ColumnIndex > 3) && (span2.End.Line.Substring(span2.End.ColumnIndex - 3, 3) == "-->")))
                    {
                        using (TextBufferLocation location = span2.End.Clone())
                        {
                            using (TextBufferLocation location2 = span2.End.Clone())
                            {
                                location.ColumnIndex -= 3;
