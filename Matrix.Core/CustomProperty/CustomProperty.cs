namespace Kingsoft.Blaze.WorldEditor.CustomPropertyGrid
{
    ...
    using Kingsoft.Blaze.WorldEditor;
    using Kingsoft.Blaze.WorldEditor.CustomPropertyGrid;
    using Kingsoft.Blaze.WorldEditor.UITypeEditors;
    ...
    public class CustomProperty
    {
        ...
        protected BrowsableTypeConverter.LabelStyle eBrowsablePropertyLabel = BrowsableTypeConverter.LabelStyle.lsNormal;
        ...
        [Category("Appearance"), DisplayName("Name"), DescriptionAttribute("Display Name of the CustomProperty."), ParenthesizePropertyName(true), XmlElementAttribute("Name")]
        public string Name
        {
            get
            {
        ...
        [Category("Appearance"), DisplayName("ReadOnly"), DescriptionAttribute("Set read only attribute of the CustomProperty."), XmlElementAttribute("ReadOnly")]
        ...
        [Category("Appearance"), DescriptionAttribute("Set visibility attribute of the CustomProperty.")]
        public bool Visible
        {
            get
            {
            ...
            get
            {
                if (bRef)
                {
                    if (oRef.GetType() == typeof(DataRow) || oRef.GetType().IsSubclassOf(typeof(DataRow)))
                        ...
                        return Interaction.CallByName(oRef, "Item", CallType.Get, sProp);
                }
                else
                {
                    return oValue;
            ...
            set
            {
                if (bRef)
                {
                    if (oRef.GetType() == typeof(DataRow) || oRef.GetType().IsSubclassOf(typeof(DataRow)))
                        ...
                        Interaction.CallByName(oRef, "Item", CallType.Set, new object[] { sProp, value });
                }
                else
                {
                    oValue = value;
        ...
        [Category("Behavior"), DescriptionAttribute("Indicates the style of the label when a property is browsable."), XmlElementAttribute(IsNullable = false)]
        ...
        public Type EditorType
        {
            get
            {
                return oCustomEditor == null ? null : oCustomEditor.GetType();
            }
            set
            {
                if (value != null)
                {
                    oCustomEditor = (UITypeEditor)Activator.CreateInstance(value);
                }
            }
        }
        ...
        //        CustomChoices choices = new CustomChoices();
        //        foreach (string choice in value.ToString().Split(",".ToCharArray()))
        //        {
        //            string[] pair = choice.Split("=".ToCharArray());
        //            if (pair.Length == 2)
        //            {
        //                ChoiceItem item = new ChoiceItem(pair[0].Trim(), pair[1].Trim());
        //                choices.Add(item);
        //            }
        //        }
        //        this.ValueMember = "Value";
        //        this.DisplayMember = "Text";
        //        this.Datasource = choices.ToArray(typeof(ChoiceItem));
        //    }
        //}
        ...
        private string m_PropertyDataSource = null;
        [XmlIgnore()]
        public string PropertyDataSource
        {
            get { return m_PropertyDataSource; }
            set
            {
                m_PropertyDataSource = value;
            }
        }
        ...
        private MultiSelectMode m_multiSelectMode;
        public MultiSelectMode MultiSelectMode
        {
            get { return m_multiSelectMode; }
            set
            {
                m_multiSelectMode = value;
            }
        }

        private string m_multiSelectSeparator = "";
        public string MultiSelectSeparator
        {
            get { return m_multiSelectSeparator; }
            set
            {
                m_multiSelectSeparator = value;
            }
        }
